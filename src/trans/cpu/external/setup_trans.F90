...
USE TPM_FFTW        ,ONLY : EC_FFTW_INIT
...
IF(.NOT. ALLOCATED(DIM_RESOL)) THEN
  ...
  ! EC_FFTW is initialized in EC_FFTW_INIT() once we know all parameters
...
CALL SET_RESOL(IDEF_RESOL,LDSETUP=.TRUE.) ! FFTW module is initialized properly below
...
IF(PRESENT(LDUSEADJOINT)) THEN
  WRITE(NOUT,*) 'LDUSEADJOINT option provided to SETUP_TRANS'
  WRITE(NOUT,*) 'THIS IS NOT IMPLEMENTE YET'
ENDIF
...
! Initialize Fast Fourier Transform package
  IF (.NOT. D%LCPNMONLY .AND. .NOT. D%LGRIDONLY) THEN
    CALL EC_FFTW_INIT(R%NDLON,NMAX_RESOL,NCUR_RESOL)
  ENDIF
  CALL GSTATS(1802,1)
ELSE
  CALL PRE_SULEG
