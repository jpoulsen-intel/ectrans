...
IF( IF_FS > 0 )THEN
  ...
  IF(.NOT.LDAVE_ONLY)THEN
    DO JF=1,IF_FS
      ZMINGL(JF,:)=ZGTF(D%NSTAGTF(1)+1,JF)
      ZMAXGL(JF,:)=ZGTF(D%NSTAGTF(1)+1,JF)
    ENDDO
  ENDIF
...
!DIR$ NEXTSCALAR
      DO JL=1,G%NLOEN(IGL)
        ZAVE(JF,JGL)=ZAVE(JF,JGL)+ZGTF(D%NSTAGTF(JGL)+JL,JF)
      ENDDO
      IF(.NOT.LDAVE_ONLY)THEN
        DO JL=1,G%NLOEN(IGL)
          ZMINGL(JF,JGL)=MIN(ZMINGL(JF,JGL),ZGTF(D%NSTAGTF(JGL)+JL,JF))
          ZMAXGL(JF,JGL)=MAX(ZMAXGL(JF,JGL),ZGTF(D%NSTAGTF(JGL)+JL,JF))
        ENDDO
      ENDIF
    ENDDO
  ENDDO
