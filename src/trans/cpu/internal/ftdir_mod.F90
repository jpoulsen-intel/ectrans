...
MODULE FTDIR_MOD
INTERFACE FTDIR
   MODULE PROCEDURE FTDIR_INOUT
END INTERFACE

CONTAINS

SUBROUTINE FTDIR_INOUT(PIN,KGL,JK)
...
IF (G%NLOEN(IGLG)>1) THEN 
  ...
  CALL EXEC_FFTW(ITYPE,IRLEN,ICLEN,IOFF,PIN(:,JK))
#else
  CALL EXEC_FFTW(ITYPE,IRLEN,ICLEN,IOFF,PIN,JK)
#endif
ENDIF
...
DO JJ=IST1,ILEN
  PIN(IST+D%NSTAGTF(KGL)+JJ-1,JK) = 0.0_JPRB
ENDDO

!     ------------------------------------------------------------------

END SUBROUTINE FTDIR_INOUT
