...
PUBLIC TRLTOG_TR, TRLTOG
PRIVATE TRLTOG_PROLOG, TRLTOG_TR_COMM, TRLTOG_TR_COMM_STACK
#else
PUBLIC TRLTOG
...
SUBROUTINE TRLTOG_TR(PGLAT_TR,KF_FS,KF_GP,KF_SCALARS_G,KVSET,KPTRGP,&
 &PGP,PGPUV,PGP3A,PGP3B,PGP2)
...
INTEGER(KIND=JPIM),INTENT(IN)  :: KF_SCALARS_G
! REAL(KIND=JPRB),INTENT(IN)     :: PGLAT(KF_FS,D%NLENGTF)
REAL(KIND=JPRB), CONTIGUOUS, INTENT(IN)     :: PGLAT_TR(:,:)
INTEGER(KIND=JPIM), INTENT(IN) :: KVSET(KF_GP)
INTEGER(KIND=JPIM) ,OPTIONAL, INTENT(IN) :: KPTRGP(:)
REAL(KIND=JPRB),OPTIONAL,INTENT(OUT),CONTIGUOUS :: PGP(:,:,:)
REAL(KIND=JPRB),OPTIONAL,INTENT(OUT)     :: PGPUV(:,:,:,:)
...
IF (LHOOK) CALL DR_HOOK('TRLTOG_TR',0,ZHOOK_HANDLE)

CALL TRLTOG_PROLOG(KF_FS,KF_GP,KVSET,&
 & ISENDCOUNT,IRECVCOUNT,INSEND,INRECV,ISENDTOT,IRECVTOT,ISEND,IRECV,IINDEX,INDOFF,IGPTRSEND, &
 & ISETAL,ISETBL,ISETWL,ISETVL)
CALL TRLTOG_TR_COMM_STACK(PGLAT_TR,KF_FS,KF_GP,KF_SCALARS_G,KVSET, &
     & ISENDCOUNT,IRECVCOUNT,INSEND,INRECV,ISENDTOT,IRECVTOT,ISEND,IRECV,IINDEX,INDOFF,IGPTRSEND, &
     & KPTRGP,PGP,PGPUV,PGP3A,PGP3B,PGP2,&
     & ISETAL,ISETBL,ISETWL,ISETVL)

IF (LHOOK) CALL DR_HOOK('TRLTOG_TR',1,ZHOOK_HANDLE)

!     ------------------------------------------------------------------

END SUBROUTINE TRLTOG_TR

SUBROUTINE TRLTOG_TR_COMM_STACK(PGLAT_TR,KF_FS,KF_GP,KF_SCALARS_G,KVSET,&
 & KSENDCOUNT,KRECVCOUNT,KNSEND,KNRECV,KSENDTOT,KRECVTOT,KSEND,KRECV,KINDEX,KNDOFF,KGPTRSEND,&
 & KPTRGP,PGP,PGPUV,PGP3A,PGP3B,PGP2, &
 & KSETAL, KSETBL,KSETWL,KSETVL)
...
CALL TRLTOG_TR_COMM(PGLAT_TR,KF_FS,KF_GP,KF_SCALARS_G,KVSET,&
 & KSENDCOUNT,KRECVCOUNT,KNSEND,KNRECV,KSENDTOT,KRECVTOT,KSEND,KRECV,KINDEX,KNDOFF,KGPTRSEND,&
 & ZCOMBUFS_STACK,ZCOMBUFR_STACK, &
 & KPTRGP,PGP,PGPUV,PGP3A,PGP3B,PGP2, &
 & KSETAL, KSETBL,KSETWL,KSETVL)

END SUBROUTINE TRLTOG_TR_COMM_STACK

SUBROUTINE TRLTOG_TR_COMM(PGLAT_TR,KF_FS,KF_GP,KF_SCALARS_G,KVSET,&
 & KSENDCOUNT,KRECVCOUNT,KNSEND,KNRECV,KSENDTOT,KRECVTOT,KSEND,KRECV,KINDEX,KNDOFF,KGPTRSEND,&
 & PCOMBUFS,PCOMBUFR, &
 & KPTRGP,PGP,PGPUV,PGP3A,PGP3B,PGP2, &
 & KSETAL, KSETBL,KSETWL,KSETVL)
...
IF (LHOOK) CALL DR_HOOK('TRLTOG_TR_BAR',0,ZHOOK_HANDLE_BAR)
CALL GSTATS_BARRIER(762)
IF (LHOOK) CALL DR_HOOK('TRLTOG_TR_BAR',1,ZHOOK_HANDLE_BAR)
...
IF (NTRANS_SYNC_LEVEL <= 0) THEN
   ...
   DO INR=1,KNRECV
      ...
      CALL MPL_RECV(PCOMBUFR(-1:KRECVTOT(IRECV),INR), &
           & KSOURCE=NPRCIDS(IRECV), &
           & KMP_TYPE=JP_NON_BLOCKING_STANDARD,KREQUEST=IREQ_RECV(INR), &
           & KTAG=ITAG,CDSTRING='TRLTOG_TR_COMM: NON-BLOCKING IRECV' )
   ENDDO
ENDIF
...
!DIR$ VECTOR ALWAYS
          DO JK=IFIRST,ILAST
            IPOS = KNDOFF(MYPROC)+IGPTROFF(JBLK)+JK-IFIRST+1
            ! PGP(JK,IFLD,JBLK) = PGLAT(JFLD64,KINDEX(IPOS))
            PGP(JK,IFLD,JBLK) = PGLAT_TR(KINDEX(IPOS),JFLD64)
          ENDDO
        ENDDO
      ELSE
        DO JFLD64=1,IFLDS
...
!DIR$ VECTOR ALWAYS
            DO JK=IFIRST,ILAST
              IPOS = KNDOFF(MYPROC)+IGPTROFF(JBLK)+JK-IFIRST+1
              ! PGPUV(JK,IUVLEVS(IFLD),IUVPARS(IFLD),JBLK) = PGLAT(JFLD64,KINDEX(IPOS))
              PGPUV(JK,IUVLEVS(IFLD),IUVPARS(IFLD),JBLK) = PGLAT_TR(KINDEX(IPOS),JFLD64)
            ENDDO
          ELSEIF(LLGP2(IFLD)) THEN
!DIR$ VECTOR ALWAYS
            DO JK=IFIRST,ILAST
              IPOS = KNDOFF(MYPROC)+IGPTROFF(JBLK)+JK-IFIRST+1
              ! PGP2(JK,IGP2PARS(IFLD),JBLK)=PGLAT(JFLD64,KINDEX(IPOS))
              PGP2(JK,IGP2PARS(IFLD),JBLK)=PGLAT_TR(KINDEX(IPOS),JFLD64)
            ENDDO
          ELSEIF(LLGP3A(IFLD)) THEN
!DIR$ VECTOR ALWAYS
            DO JK=IFIRST,ILAST
              IPOS = KNDOFF(MYPROC)+IGPTROFF(JBLK)+JK-IFIRST+1
              ! PGP3A(JK,IGP3ALEVS(IFLD),IGP3APARS(IFLD),JBLK)=PGLAT(JFLD64,KINDEX(IPOS))
              PGP3A(JK,IGP3ALEVS(IFLD),IGP3APARS(IFLD),JBLK)=PGLAT_TR(KINDEX(IPOS),JFLD64)
            ENDDO
          ELSEIF(LLGP3B(IFLD)) THEN
!DIR$ VECTOR ALWAYS
            DO JK=IFIRST,ILAST
              IPOS = KNDOFF(MYPROC)+IGPTROFF(JBLK)+JK-IFIRST+1
              ! PGP3B(JK,IGP3BLEVS(IFLD),IGP3BPARS(IFLD),JBLK)=PGLAT(JFLD64,KINDEX(IPOS))
              PGP3B(JK,IGP3BLEVS(IFLD),IGP3BPARS(IFLD),JBLK)=PGLAT_TR(KINDEX(IPOS),JFLD64)
            ENDDO
          ELSE
            WRITE(NOUT,*)'TRLTOG_TR_MOD: ERROR',JFLD64,IFLD
            CALL ABORT_TRANS('TRLTOG_TR_MOD: ERROR')
          ENDIF
        ENDDO
      ENDIF
    ENDIF
...
DO INS=1,KNSEND
  ...
  DO JFLD=ISEND_FLD_START,ISEND_FLD_END
     DO JL=1,ILEN
        II = KINDEX(KNDOFF(ISEND)+JL)
        PCOMBUFS((JFLD-ISEND_FLD_START)*ILEN+JL,INS) = PGLAT_TR(II,JFLD)
     ENDDO
  ENDDO
  !$OMP END PARALLEL DO
  PCOMBUFS(-1,INS) = 1
