...
REAL(KIND=JPRB), INTENT(IN), CONTIGUOUS    :: PSOA(:,:)
REAL(KIND=JPRB), INTENT(IN), CONTIGUOUS    :: PAOA(:,:)

!     LOCAL INTEGERS
INTEGER(KIND=JPIM) :: ISL, IGLS, JFLD, JGL ,IPROC, IPROCS, IDGNH
INTEGER(KIND=JPIM) :: OFFS1, OFFS2
...
!DIR$ IVDEP
  DO JFLD=1,2*KFIELD
    FOUBUF_IN(OFFS1+JFLD) = PAOA(JFLD,JGL)+PSOA(JFLD,JGL)
    FOUBUF_IN(OFFS2+JFLD) = PSOA(JFLD,JGL)-PAOA(JFLD,JGL)
  ENDDO
ENDDO
