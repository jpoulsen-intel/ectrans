...
USE TPM_TRANS       ,ONLY : LDIVGP, LSCDERS, LUVDER, LVORGP,LATLON, REALLOCATE_ARRAY_PAD64BYTE
USE TPM_DISTR       ,ONLY : D
USE TPM_FLT         ,ONLY : S
USE FOURIER_IN_MOD  ,ONLY : FOURIER_IN ! _1D
USE FSC_MOD         ,ONLY : FSC, FSC_TR, FSC_TR_SCALAR, FSC_TR_UV, FSC_SCALAR_1D, FSC_UV_1D
USE FTINV_MOD       ,ONLY : FTINV_1D
USE TRLTOG_MOD      ,ONLY : TRLTOG, TRLTOG_TR
...
REAL(KIND=JPRB),OPTIONAL    ,INTENT(OUT), CONTIGUOUS :: PGP(:,:,:)
...
INTEGER(KIND=JPIM) :: JF, JJF
! INTEGER(KIND=JPIM) :: KF_OUT_IND, KF_UV_IND, KF_SCALAR_IND, KF_NSDERS_IND, KF_UVDERS_IND, KF_EWDERS_IND

REAL(KIND=JPRB),POINTER, CONTIGUOUS :: ZUV_TR(:,:)
REAL(KIND=JPRB),POINTER, CONTIGUOUS :: ZSCALAR_TR(:,:)
REAL(KIND=JPRB),POINTER, CONTIGUOUS :: ZNSDERS_TR(:,:)
REAL(KIND=JPRB),POINTER, CONTIGUOUS :: ZEWDERS_TR(:,:)
REAL(KIND=JPRB),POINTER, CONTIGUOUS :: ZUVDERS_TR(:,:)

REAL(KIND=JPRB), TARGET, ALLOCATABLE,SAVE :: ZGTF_TR(:,:)
REAL(KIND=JPRB), TARGET :: ZDUM(1,1)
! REAL(KIND=JPRB),POINTER,CONTIGUOUS  :: ZGTF_TR(:,:)

LOGICAL :: LDERIV

! Allow deferencing ZUV_TR, ZSCALAR_TR, ZNSDERS_TR, ZEWDERS_TR, ZUVDERS_TR
ZUV_TR     => ZDUM
ZSCALAR_TR => ZDUM
ZNSDERS_TR => ZDUM
ZEWDERS_TR => ZDUM
...
! Allocate from heap and pad first dimension to 64 byte boundaries (needed by FFTW)
CALL REALLOCATE_ARRAY_PAD64BYTE(ZGTF_TR, D%NLENGTF,KF_FS, REAL(0,JPRB))

LDERIV=(KF_UV > 0 .OR. KF_SCDERS > 0 .OR.  (LATLON.AND.S%LDLL))
...
IF (LDERIV) THEN
  ...
  ENDIF
  IF (KF_UV>0) THEN
     ZUV_TR => ZGTF_TR(:,IST:IST+2*KF_UV-1)
     ! KF_UV_IND=IST
  END IF
  IST = IST+2*KF_UV
  IF (KF_SCALARS>0) THEN
     ZSCALAR_TR => ZGTF_TR(:,IST:IST+KF_SCALARS-1)
     ! KF_SCALAR_IND=IST
  END IF
  IST = IST+KF_SCALARS
  IF (KF_SCDERS>0) THEN
     ZNSDERS_TR => ZGTF_TR(:,IST:IST+KF_SCDERS-1)
     ! KF_NSDERS_IND=IST
  END IF
  IST = IST+KF_SCDERS
  IF (LUVDER) THEN
     ZUVDERS_TR => ZGTF_TR(:,IST:IST+2*KF_UV-1)
     ! KF_UVDERS_IND=IST
     IST = IST+2*KF_UV
  ENDIF
  IF (KF_SCDERS > 0) THEN
     ZEWDERS_TR => ZGTF_TR(:,IST:IST+KF_SCDERS-1)
     ! KF_EWDERS_IND=IST
  ENDIF
ENDIF
...
! Loop over latitudes
!$OMP PARALLEL DO PRIVATE(JGL,IGL,JF,JJF) SCHEDULE(DYNAMIC,1)
DO JGL = 1, D%NDGL_FS
   IGL = JGL
   ! Gather all the field values
   DO JF=1,KF_OUT_LT
      CALL FOURIER_IN(ZGTF_TR(:,JF),KF_OUT_LT,IGL,JF)
   END DO

   !    2.  Fourier space computations
   IF (LDERIV) THEN
      DO JF=1,KF_SCALARS
         CALL FSC_SCALAR_1D(IGL,KF_SCALARS,KF_SCDERS,ZSCALAR_TR(:,JF),ZNSDERS_TR(:,JF),ZEWDERS_TR(:,JF))
      END DO
      DO JF=1,2*KF_UV
         CALL FSC_UV_1D(IGL,KF_UV,ZUV_TR(:,JF),ZUVDERS_TR(:,JF))
      END DO
   END IF
      
   ! 3. .  Fourier transform
   DO JF=1,KF_FS
      CALL FTINV_1D(ZGTF_TR(:,JF),KF_FS,IGL)
   END DO
END DO
!$OMP END PARALLEL DO
...
CALL TRLTOG_TR(ZGTF_TR,KF_FS,KF_GP,KF_SCALARS_G,IVSET,KPTRGP,&
     &PGP,PGPUV,PGP3A,PGP3B,PGP2)
...
IF (.NOT. LALLOPERM) DEALLOCATE(ZGTF_TR)
